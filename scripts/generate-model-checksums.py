...
vosk_model_info.py, whispercpp_model_info.py, parakeet_model_info.py or
faster_whisper_model_info.py is picked up on the next run and a model that
upstream never published fails the generation loudly.
...
def _parakeet_entries() -> list[Entry]:
    """Digests for every file of every Parakeet bundle, at its pinned revision.

    The ONNX files are LFS, so Hugging Face publishes their sha256; tokens.txt
    is a plain blob and is hashed from the bytes this script downloads, the same
    way VOSK archives are.
    """
    sys.path.insert(0, str(SRC))
    ...
    entries = []
    for model_name, info in PARAKEET_MODEL_INFO.items():
        repo, revision = info["repo"], info["revision"]
        payload = _fetch_json(
...
def _faster_whisper_entries() -> list[Entry]:
    """Digests for every file of every faster-whisper bundle, at its pinned revision.

    ``model.bin`` is LFS (Hugging Face publishes sha256 on these repos); config,
    tokenizer and vocabulary files are plain blobs hashed from the bytes this
    script downloads, the same way Parakeet's tokens.txt is.
    """
    sys.path.insert(0, str(SRC))
    ...
    entries = []
    for model_name, info in FASTER_WHISPER_MODEL_INFO.items():
        repo, revision = info["repo"], info["revision"]
        payload = _fetch_json(
            f"https://huggingface.co/api/models/{repo}/revision/{revision}?blobs=true"
        )
...
def _render(entries: list[Entry], revision: str) -> str:
    ...
    lines = [
        ...
        "# vosk_model_info.py, whispercpp_model_info.py, parakeet_model_info.py",
        "# or faster_whisper_model_info.py.",
        "#",
        ...
        "#",
        "# Parakeet and faster-whisper bundles are pinned per model in their",
        "# model_info modules, so their keys carry the model name: several",
        "# bundles ship the same file names.",
        "",
    ]
...
def main() -> int:
    ...
    parakeet_entries = _parakeet_entries()
    print(f"  Parakeet: {len(parakeet_entries)} sha256 digests")

    faster_whisper_entries = _faster_whisper_entries()
    print(f"  faster-whisper: {len(faster_whisper_entries)} sha256 digests")

    MANIFEST.write_text(
        _render(
            whispercpp_entries + vosk_entries + parakeet_entries + faster_whisper_entries,
            revision,
        ),
        encoding="utf-8",
    )
