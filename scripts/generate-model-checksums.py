...
def _render(entries: list[Entry], revision: str) -> str:
    ...
    lines = [
        ...
        "# vosk_model_info.py, whispercpp_model_info.py or parakeet_model_info.py.",
        "#",
        ...
        "#",
        "# Parakeet bundles are pinned per model in parakeet_model_info.py, so their",
        "# keys carry the model name: several bundles ship the same file names.",
        "",
    ]
...
def main() -> int:
    ...
    MANIFEST.write_text(
        _render(whispercpp_entries + vosk_entries + parakeet_entries, revision), encoding="utf-8"
    )
